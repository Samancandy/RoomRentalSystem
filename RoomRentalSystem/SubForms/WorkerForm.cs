...
namespace RoomRentalSystem
{
    public partial class WorkerForm : UserControl
    {
        ...
        internal void SetWorker(string txt, Person w)
        {
            btnUpdate.Text = txt;
            if (w != null)
            {
        ...
        private void HandleData(Person person)
        {
            ...
            if (p == null)
            {
                sql = string.Format("insert into tbPerson values('{0}','{1}', '{2}', '{3}', '{4}', '{5}')",
                    txtWorkerID.Text, txtCustomerName.Text, cboGender.Text, dtpDateBirth.Value, txtPhone.Text, txtAddress.Text);
                message = "Successfully Added!"; toClear = "yes";
            }
            else
            {
                sql = string.Format("update tbPerson set Name = '{0}', Gender = '{1}', DOB = '{2}', Phone = '{3}', Addr = '{4}' where PersonID = '{5}'",
                    txtCustomerName.Text, cboGender.Text, dtpDateBirth.Value, txtPhone.Text, txtAddress.Text, txtWorkerID.Text);
